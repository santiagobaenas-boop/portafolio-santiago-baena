...
const categories: Array<'All' | VideoCategory> = [
  'All',
  'Reels',
  'Automotive',
  'Brand',
...
function VideoCard({
  ...
  return (
    <button
      ...
      aria-label={`Watch video ${video.titulo}`}
    >
      {/* 16:9 thumbnail area */}
...
export default function EdicionVideo() {
  const [active, setActive] = useState<'All' | VideoCategory>('All');
  const [selected, setSelected] = useState<Video | null>(null);

  const filtered =
    active === 'All' ? videos : videos.filter(v => v.categoria === active);
